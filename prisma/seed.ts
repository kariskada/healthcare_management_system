...
async function seed() {
  ...
  const doctors = [];
  for (let i = 0; i < 10; i++) {
    // Pick 3–6 random days from the week
    const selectedDays = faker.helpers.arrayElements(daysOfWeek, faker.number.int({ min: 3, max: 6 }));

    // Create working days with random hours
    const workingDays = selectedDays.map((day: string) => {
      const startHour = faker.number.int({ min: 8, max: 10 }); // 8 AM - 10 AM
      const endHour = startHour + faker.number.int({ min: 6, max: 8 }); // Work 6–8 hrs
